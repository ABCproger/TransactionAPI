...
public class TransactionService : ITransactionService
{
    ...
    public async Task<IEnumerable<TransactionDTO>> GetTransactionsBetweenDates(DateTime startDate, DateTime endDate)
    {
        ...
        const string sql = @"
        SELECT transaction_id, name, email, amount, transaction_date_local, transaction_date_utc, client_location, time_zone_rules 
        FROM ""Transactions"" 
        WHERE transaction_date_local BETWEEN @StartDate AND @EndDate";

        var parameters = new
        {
            StartDate = LocalDateTime.FromDateTime(startDate).ToDateTimeUnspecified(),
            EndDate = LocalDateTime.FromDateTime(endDate).ToDateTimeUnspecified()
        };
